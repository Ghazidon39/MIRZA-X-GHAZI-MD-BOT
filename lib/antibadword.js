...
async function handleBadwordDetection(sock, chatId, message, userMessage, senderId) {
   ...
   // console.log('Bad word detected in:', userMessage);
    ...
    switch (antiBadwordConfig.action) {
        case 'delete':
            await sock.sendMessage(chatId, {
                text: `*@${senderId.split('@')[0]} MERI JAAN YAHAN BAD WORDS ALLOWED NHI HAIN IS LIYE NA USE KRO*`,
                mentions: [senderId]
            });
            break;
        ...
        case 'kick':
            try {
                ...
                await sock.sendMessage(chatId, {
                    text: `*@${senderId.split('@')[0]} NE BAD WORDS USE KIYE IS LIYE OS KO KICK KR DIA *`,
                    mentions: [senderId]
                });
            } catch (error) {
        ...
        case 'warn':
            ...
            if (warningCount >= 3) {
                try {
                    ...
                    await sock.sendMessage(chatId, {
                        text: `*@${senderId.split('@')[0]} IS KO PEHLE 3 WARNINGS DEIIN OR FIR KICK KR DIA*`,
                        mentions: [senderId]
                    });
                } catch (error) {
